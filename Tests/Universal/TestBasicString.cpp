...
TEST_CASE(create)
{
    ...
    CHECK_EQUAL((size_t)0, empty_string.length());

    BasicString<char> s("Small");
    CHECK_EQUAL((size_t)5, s.length());
    CHECK_STR_EQUAL("Small", s.str());

    BasicString<char> l("Larger string heap allocation");
    CHECK_EQUAL((size_t)29, l.length());
...
TEST_CASE(move_and_copy)
{
    ...
    CHECK_EQUAL((size_t)5, s.length());
    CHECK_STR_EQUAL("Small", s.str());
    CHECK_EQUAL((size_t)5, s_copy.length());
    CHECK_STR_EQUAL("Small", s_copy.str());

    BasicString<char> s_move(move(s));
    CHECK_EQUAL((size_t)0, s.length());
    CHECK_STR_EQUAL("", s.str());
    CHECK_EQUAL((size_t)5, s_move.length());
